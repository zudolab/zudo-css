...
コンポーネントが段階的に広いデフォルトを確認するレイヤード設定システムを作成します。これは[3層カラー戦略](/docs/styling/color/three-tier-color-strategy)の背後にあるメカニズムです — コンポーネントトークンはテーマトークンにフォールバックし、テーマトークンはパレット値にフォールバックします。
