...
<Info title="Tailwind + component-first projects">
In component-first projects (React, Vue, Svelte with Tailwind), these arbitrary values are written directly as bracket syntax in JSX — `w-[28px]`, `grid-cols-[240px_1fr]`. No component-scoped CSS custom properties are needed because the component file itself provides scoping. In general CSS projects (BEM, CSS Modules), these same one-off values can be stored as component-scoped custom properties with an underscore prefix: `--_button-width: 28px`.
</Info>
...
- [Three-Tier Color Strategy](../color/three-tier-color-strategy/) — Full three-tier architecture for colors
- [Three-Tier Font-Size Strategy](../typography/three-tier-font-size-strategy/) — Three-tier architecture for font sizes
