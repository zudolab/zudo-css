...
<Info title="Component-scoped CSS custom properties">
The demos above express arbitrary values as hardcoded CSS or Tailwind bracket syntax. In general CSS projects (BEM, CSS Modules), these component-specific values are often stored in CSS custom properties scoped to the component selector. When doing so, use an underscore prefix to signal local scope — see [Naming Convention for Component-Scoped Variables](#naming-convention-for-component-scoped-variables) below.
</Info>
