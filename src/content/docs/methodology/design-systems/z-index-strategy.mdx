...
| Raw integer with `/* lint-ignore-z-index */` escape hatch | Bare raw integer with no escape comment |
...
A companion lint-rule article (covering AST patterns, CI wiring, and the matching enforcement rules for other token families) will land in a follow-up sub-issue.
